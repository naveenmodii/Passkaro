    ...
    return (
      <div className="min-h-[calc(100vh-4rem)] bg-zinc-50 text-zinc-900 flex items-center justify-center p-4">
        <div className="bg-white border border-zinc-300 p-8 rounded-2xl max-w-md text-center shadow-sm">
          ...
          <Link to={`/college/${slug}`} className="text-xs font-bold bg-zinc-900 text-white px-4.5 py-2.5 rounded-xl hover:bg-black transition-all shadow-sm">
            ← Back to Branches
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="min-h-[calc(100vh-4rem)] bg-zinc-50 text-zinc-900 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        {/* Header Breadcrumb */}
        <div>
          <Link to={`/college/${slug}`} className="text-zinc-600 hover:text-zinc-900 text-xs font-bold tracking-wider uppercase transition-colors">
            ← Back to Branches
          </Link>
        ...
        {semesters.length > 0 && (
          <div className="flex items-center space-x-2.5 overflow-x-auto pb-2 border-b border-zinc-300">
            <button
              onClick={() => setSelectedSemester('all')}
              className={`px-4.5 py-2 rounded-xl text-xs font-bold transition-all whitespace-nowrap shadow-2xs ${
                selectedSemester === 'all'
                  ? 'bg-zinc-900 text-white shadow-sm ring-1 ring-zinc-900/10'
                  : 'bg-white text-zinc-700 border border-zinc-300 hover:bg-zinc-100 hover:text-zinc-900'
              }`}
            >
              All Semesters
            </button>
                ...
                >
                  <div>
                    <div className="flex items-center justify-between mb-3">
                      <span className="text-xs font-mono font-bold text-zinc-800 bg-zinc-100 border border-zinc-200 px-2.5 py-1 rounded-md">
                        Sem {subject.semester}
                      </span>

                      {/* Locked/Unlocked Badge without emojis */}
                      {unlocked ? (
                        <span className="inline-flex items-center text-xs font-bold text-emerald-900 bg-emerald-100/80 border border-emerald-300 px-3 py-1 rounded-full">
                          Unlocked
                        </span>
                      ) : (
                        <span className="inline-flex items-center text-xs font-bold text-zinc-800 bg-zinc-100 border border-zinc-300 px-3 py-1 rounded-full">
                          Locked
                        </span>
                      )}
                    </div>
                    ...
                    <div className="text-xs text-zinc-600 font-semibold mt-2">
                  ...
                  <div className="mt-6 pt-4 border-t border-zinc-200 flex items-center justify-between">
                    ...
                    <span className="text-xs font-bold text-white bg-zinc-900 group-hover:bg-black px-3.5 py-1.5 rounded-xl transition-all shadow-2xs">
                      View Course →
                    </span>
                  </div>
                </Link>
